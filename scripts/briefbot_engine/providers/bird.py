...
def _get_x_cookie_env() -> Dict[str, str]:
    """Build env dict with AUTH_TOKEN/CT0 for bird-search subprocess.

    Resolution order:
    1. Already in environment variables
    2. In briefbot's .env config file (written by cookie-bridge Chrome extension)

    Chrome 127+ uses App-Bound Encryption which prevents reading cookies from
    Python. The cookie-bridge Chrome extension reads decrypted cookies via the
    chrome.cookies API and writes them to ~/.config/briefbot/.env via native
    messaging. See briefbot_engine/cookie-bridge/ for setup instructions.
    """
    extra_env = {}
    ...
    try:
        from briefbot_engine.config import CONFIG_FILE, parse_dotenv
...
# Cache the env dict to avoid repeated expensive COM calls
_cached_env: Optional[Dict[str, str]] = None


def _build_env() -> Dict[str, str]:
    """Build environment dict for bird-search subprocess (cached)."""
    global _cached_env
    if _cached_env is None:
        _cached_env = os.environ.copy()
        _cached_env.update(_get_x_cookie_env())
    return _cached_env
...
def is_bird_authenticated() -> Optional[str]:
    """Check if X credentials are available (env vars, config file, or browser cookies).
...
def install_bird() -> Tuple[bool, str]:
    """No-op - Bird search is vendored, no installation needed.
...
def search_handles(
    ...
    Used in supplemental search after entity extraction.
