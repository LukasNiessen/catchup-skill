...
- **env.py**: Load and validate API keys from `~/.config/catchup/.env`
...
```
python3 ~/.claude/skills/catchup/scripts/catchup.py <topic> [options]
...
All outputs are written to `~/.local/share/catchup/out/`:

- `report.md` - Human-readable full report
- `report.json` - Normalized data with scores
- `catchup.context.md` - Compact reusable snippet for other skills
